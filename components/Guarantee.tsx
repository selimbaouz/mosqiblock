...
const Guarantee = () => {
    ...
    return (
        <div className={cn("bg-tertiary w-full")}>
            <div className={cn("max-w-screen-xl mx-auto flex flex-col lg:grid lg:grid-cols-3 items-center justify-between gap-2 lg:gap-4 px-4 py-10 lg:px-0 g:py-20")}>
                {items.map((data, index) => (
                    <div key={index} className={cn("bg-white rounded-3xl size-full flex flex-col pt-6 justify-center items-center space-y-2")}>
                        <Image src={data.img.src} alt="Icon of guarantee" width={data.img.width} height={data.img.height} className={cn("w-auto h-20 lg:h-32")} />
